...
let rec add ty1 ty2 mem = match (ty1, ty2) with
  ...
  | (TyLambda _, _) -> (singleton (TyNotImplemented), mem)
  | (TyAFunction _, _) -> (singleton (TyNotImplemented), mem)
  (* Generator *)
...
let rec sub ty1 ty2 mem = match (ty1, ty2) with
  (* Bool *)
  | (TyCBool true, _)  -> sub (TyCInt 1) ty2 mem
  | (TyCBool false, _) -> sub (TyCInt 0) ty2 mem
  | (_, TyCBool true)  -> sub ty1 (TyCInt 1) mem
  | (_, TyCBool false) -> sub ty1 (TyCInt 0) mem
  | (TyBool, _) -> sub TyInt ty2 mem
  | (_, TyBool) -> sub ty1 TyInt mem
  (* CInt *)
  | (TyCInt n, TyCInt m) -> (singleton (TyCInt (n - m)), mem)
  | (TyCInt n, TyInt) -> (singleton (TyInt), mem)
  | (TyCInt n, _) -> (singleton (TyNotImplemented), mem)
  ...
  | (TyAByteArray, _) -> (singleton (TyNotImplemented), mem)
  (* Set *)
  | (TySet (addrset1, _), TySet _) -> (singleton (TyASet addrset1), mem)
  | (TySet (addrset1, _), TyASet _) -> (singleton (TyASet addrset1), mem)
  | (TySet _, _) -> (singleton (TyNotImplemented), mem)
  (* ASet *)
  | (TyASet _, _) -> (singleton (TyNotImplemented), mem)
  (* FrozenSet *)
